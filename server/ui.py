...
def create_ui():
    ...
    with gr.Blocks(theme=theme, title="PullRequest Arena") as app:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=4):
                ...
                action_type = gr.Radio(
                    choices=["approve", "request_changes", "comment", "suggest_fix"],
                    label="Decision",
                    value="request_changes"
                )
                action_comment = gr.Textbox(label="Review Comment (Provide explicit reasoning)", lines=6, placeholder="I noticed that logging the card details directly here introduces a severe PCI-DSS violation...")
        ...
        def submit_review(env, a_type, a_comment):
            if env is None:
                return env, "", "ERROR: Please load a PR Scenario first!"
            
            action = ReviewAction(type=a_type, comment=a_comment)
            obs = env.step(action)
        ...
        submit_btn.click(
            submit_review,
            inputs=[env_state, action_type, action_comment],
